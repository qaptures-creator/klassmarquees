        ...
        {hasTestimonials ? (
          <div className="mt-14 space-y-14">
            {testimonials.map((t) => (
              <Reveal key={t.quote}>
                <blockquote>
                  <p className="text-balance font-serif text-display italic text-ink">&ldquo;{t.quote}&rdquo;</p>
                  <footer className="mt-6 text-sm font-semibold uppercase tracking-[0.14em] text-bronze">
                    {t.attribution}
                    {t.context ? <span className="text-ink/50"> — {t.context}</span> : null}
                  </footer>
                </blockquote>
              </Reveal>
            ))}
