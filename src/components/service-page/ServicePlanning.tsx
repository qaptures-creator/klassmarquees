        ...
        <div className="mt-16 space-y-0 border-t border-ink/10">
          {content.planningSteps.map((step, i) => (
            <Reveal key={step.index} delay={i * 0.06}>
              <div className="flex flex-col gap-3 border-b border-ink/10 py-8 sm:flex-row sm:items-baseline sm:gap-10">
                <span className="w-12 shrink-0 font-serif text-h3 text-bronze">{step.index}</span>
                <div>
                  <h3 className="font-serif text-h3 font-medium text-ink">{step.title}</h3>
                  <p className="mt-2 max-w-2xl text-sm leading-relaxed text-ink/65">{step.description}</p>
                </div>
              </div>
            </Reveal>
          ))}
